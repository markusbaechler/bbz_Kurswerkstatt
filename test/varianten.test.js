...
/* Schritt 3 fuehrt laut Kontrakt zwei Entwuerfe nebeneinander. */
...
test('ohne Varianten im Kontrakt bleibt alles wie bisher', () => {
  assert.strictEqual(inhalt.varianten(INHALT, 2), null);
  assert.strictEqual(inhalt.lieferobjektVon(INHALT, 2), 'lernziele-drehbuch');
});

test('der Kontrakt gibt die erlaubten Varianten vor', () => {
  assert.deepStrictEqual(inhalt.varianten(mitVarianten(), 3), ['claude', 'chatgpt']);
});

test('der Platzhalter wird durch die Variante ersetzt', () => {
  const i = mitVarianten();
  assert.strictEqual(inhalt.lieferobjektVon(i, 3, 'claude'), 'greenfield-claude');
  assert.strictEqual(inhalt.lieferobjektVon(i, 3, 'chatgpt'), 'greenfield-chatgpt');
});

/* Der Fehler, den das verhindern soll: {variante} woertlich im Dateinamen. */
test('ohne Variante entsteht KEIN Dateiname', () => {
  const i = mitVarianten();
  assert.strictEqual(inhalt.lieferobjektVon(i, 3), null);
  assert.strictEqual(inhalt.naechsteDatei(i, 3, 'AFL-001', []), null);
  assert.strictEqual(inhalt.hochladeZiel(i, 3, 'AFL-001', [], undefined), null);
});

test('eine erfundene Variante wird nicht akzeptiert', () => {
  const i = mitVarianten();
  assert.strictEqual(inhalt.lieferobjektVon(i, 3, 'gemini'), null);
  assert.strictEqual(inhalt.hochladeZiel(i, 3, 'AFL-001', [], 'gemini'), null);
});

test('der Zielname traegt die Variante', () => {
  const z = inhalt.hochladeZiel(mitVarianten(), 3, 'AFL-001', [], 'claude');
...
test('jede Variante fuehrt ihre eigene Nummernreihe', () => {
  ...
  assert.strictEqual(inhalt.hochladeZiel(i, 3, 'AFL-001', da, 'claude').datei,
                     'AFL-001_greenfield-claude_v3.html');
  assert.strictEqual(inhalt.hochladeZiel(i, 3, 'AFL-001', da, 'chatgpt').datei,
...
test('die Endung kommt aus dem Kontrakt', () => {
  assert.strictEqual(inhalt.erwarteteEndung(mitVarianten(), 3), 'html');
});

/* ---------- Ansicht ---------- */

test('Schritt 3 bietet die Variantenwahl an', () => {
  const h = ansichten.einSchritt(mitVarianten(), AFL, 3, null,
...
test('der angezeigte Zielname folgt der gewaehlten Variante', () => {
  const i = mitVarianten();
  const a = ansichten.einSchritt(i, AFL, 3, null,
...
test('Schritte ohne Varianten zeigen keine Wahl', () => {
  ...
  assert.ok(/data-action="hochladen"/.test(h), 'kein Upload auf Schritt 2');
  assert.ok(!/data-action="variante"/.test(h), 'Variantenwahl ohne Varianten im Kontrakt');
});

/* ---------- Der Weg Chat ----------
   Er war auf Schritt 3 funktionslos: Zielname und Sperre wurden ohne Variante
...
test('ohne Varianten im Kontrakt gibt es keine gewaehlte Variante', () => {
  assert.strictEqual(inhalt.gewaehlteVariante(INHALT, 2), undefined);
});

test('ohne getroffene Wahl gilt die erste Variante des Kontrakts', () => {
  assert.strictEqual(inhalt.gewaehlteVariante(mitVarianten(), 3), 'claude');
  assert.strictEqual(inhalt.gewaehlteVariante(mitVarianten(), 3, null), 'claude');
});

test('eine getroffene Wahl gilt', () => {
  assert.strictEqual(inhalt.gewaehlteVariante(mitVarianten(), 3, 'chatgpt'), 'chatgpt');
});

test('eine erfundene Wahl faellt auf die erste zurueck', () => {
  assert.strictEqual(inhalt.gewaehlteVariante(mitVarianten(), 3, 'gemini'), 'claude');
});
